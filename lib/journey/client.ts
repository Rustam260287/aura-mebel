...
export function trackJourneyEvent(input: JourneyEventInput): void {
  ...
  // ========== YANDEX METRIKA / DIRECT INTEGRATION ==========
  try {
    if (typeof window !== 'undefined') {
      ...
      if (ym) {
        const ymid = 106314786;

        // E-commerce & Goals for Yandex Direct optimization
        switch (input.type) {
          case 'VIEW_OBJECT':
            ym(ymid, 'reachGoal', 'view_item');
            if (input.objectId) {
              dataLayer.push({
                ecommerce: {
                  detail: {
                    products: [{ id: input.objectId, name: `Object ${input.objectId}` }]
                  }
                }
              });
            }
          ...
          case 'START_AR':
            ym(ymid, 'reachGoal', 'ar_start');
            break;

          case 'SAVE_OBJECT':
            ym(ymid, 'reachGoal', 'add_to_saved');
            if (input.objectId) {
              dataLayer.push({
                ecommerce: {
                  add: {
                    products: [{ id: input.objectId, name: `Object ${input.objectId}`, quantity: 1 }]
                  }
                }
              });
            }
          ...
          case 'CONTACT_MANAGER':
            ym(ymid, 'reachGoal', 'handoff_request');
            break;

          case 'HANDOFF_SENT':
            ym(ymid, 'reachGoal', 'handoff_sent');
            dataLayer.push({
              ecommerce: {
                purchase: {
                  actionField: {
                    id: input.meta?.handoffId || `lead_${Date.now()}`
                  },
                  products: [{ id: input.objectId || 'unknown_item', name: input.meta?.handoff?.objectName || 'Lead Submission', quantity: 1 }]
                }
              }
            });
            break;
