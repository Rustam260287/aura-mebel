
import React from 'react';
import type { ObjectAdmin } from '../../types';
import { JobManager } from './JobManager';

interface AdminDashboardProps {
  objects: ObjectAdmin[];
  onAutoFixProblems?: (ids: string[]) => Promise<void>;
}

const StatCard: React.FC<{ title: string; value: string | number; children: React.ReactNode }> = ({ title, value, children }) => (
    <div className="bg-white p-6 rounded-lg shadow-md flex items-center">
        <div className="bg-brand-cream p-4 rounded-full">
            {children}
        </div>
        <div className="ml-4">
            <p className="text-lg font-semibold text-brand-charcoal">{title}</p>
            <p className="text-3xl font-serif text-brand-brown">{value}</p>
        </div>
    </div>
);

const buildProblemReason = (object: ObjectAdmin) => {
  const reasons: string[] = [];
  if (!object.imageUrls || object.imageUrls.length === 0) {
    reasons.push('нет фото');
  }
  const descLength = (object.description || '').replace(/\s+/g, ' ').trim().length;
  if (!object.description || descLength < 100) {
    reasons.push('короткое описание');
  }
  if (!object.modelGlbUrl && !object.modelUsdzUrl) {
    reasons.push('нет 3D');
  }
  return reasons.join(', ');
};

const hasStrangeDescription = (description?: string) => {
  if (!description) return false;
  return /уточните.+консультант/iu.test(description);
};

interface ProblemListProps {
  title: string;
  objects: ObjectAdmin[];
  emptyLabel: string;
  onEditObject?: (object: ObjectAdmin) => void;
  getReason: (object: ObjectAdmin) => string;
}

const ProblemList: React.FC<
  ProblemListProps & { onAutoFixProblemsClick?: (ids: string[]) => void }
> = ({ title, objects, emptyLabel, onEditObject, getReason, onAutoFixProblemsClick }) => (
  <div className="bg-white p-5 rounded-lg shadow-md">
    <div className="flex items-center justify-between mb-3">
      <div className="flex items-center gap-2">
        <h2 className="text-lg font-semibold text-brand-charcoal">{title}</h2>
        <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">
          {objects.length}
        </span>
      </div>
      {objects.length > 0 && onAutoFixProblemsClick && (
        <button
          type="button"
          onClick={() => onAutoFixProblemsClick(objects.map(p => p.id))}
          className="text-xs font-semibold text-brand-brown hover:text-brand-charcoal px-3 py-1 rounded-full border border-brand-brown/30 hover:border-brand-charcoal/40 transition-colors"
        >
          Исправить автоматически
        </button>
      )}
    </div>
    {objects.length === 0 ? (
      <p className="text-sm text-gray-400">{emptyLabel}</p>
    ) : (
      <ul className="divide-y divide-gray-100">
        {objects.slice(0, 8).map((object) => (
          <li key={object.id} className="py-2.5 flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm font-medium text-brand-charcoal truncate">{object.name}</p>
              <p className="text-xs text-gray-500 truncate">
                {getReason(object)}
              </p>
            </div>
            {onEditObject && (
              <button
                type="button"
                onClick={() => onEditObject(object)}
                className="text-xs font-semibold text-brand-brown hover:text-brand-charcoal px-3 py-1 rounded-full border border-brand-brown/30 hover:border-brand-charcoal/40 transition-colors"
              >
                Открыть
              </button>
            )}
          </li>
        ))}
      </ul>
    )}
  </div>
);

export const AdminDashboard: React.FC<
  AdminDashboardProps & { onEditObject?: (object: ObjectAdmin) => void }
> = ({ objects, onEditObject, onAutoFixProblems }) => {
  const problemObjects = objects.filter((p) => {
    const hasImages = p.imageUrls && p.imageUrls.length > 0;
    const has3D = Boolean(p.modelGlbUrl || p.modelUsdzUrl);
    const descLength = (p.description || '').replace(/\s+/g, ' ').trim().length;
    return !hasImages || !has3D || descLength < 100;
  });

  const strangeObjects = objects.filter((p) => {
    return hasStrangeDescription(p.description);
  });

  const objectsWith3D = objects.filter((o) => Boolean(o.modelGlbUrl || o.modelUsdzUrl)).length;
  const readyObjects = objects.filter((o) => o.status === 'ready').length;

  return (
    <div>
      <div className="mb-8">
        <JobManager />
      </div>

      <h1 className="text-3xl font-serif text-brand-brown mb-8">Обзор</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <StatCard title="Всего объектов" value={objects.length}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-brand-brown" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>
        </StatCard>
        <StatCard title="С 3D/AR" value={objectsWith3D}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-brand-brown" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 8V4m0 16v-4m8-4h-4M8 12H4m12.364 6.364l-2.828-2.828M9.464 8.464 6.636 5.636m12.728 0-2.828 2.828M9.464 15.536l-2.828 2.828" /></svg>
        </StatCard>
        <StatCard title="Ready" value={readyObjects}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-brand-brown" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" /></svg>
        </StatCard>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <ProblemList
          title="Проблемные объекты"
          objects={problemObjects}
          emptyLabel="Все объекты выглядят хорошо — 3D, фото и тексты на месте."
          onEditObject={onEditObject}
          getReason={buildProblemReason}
          onAutoFixProblemsClick={
            onAutoFixProblems
              ? (ids: string[]) => {
                  onAutoFixProblems(ids).catch(err => {
                    console.error('Auto-fix problems failed', err);
                  });
                }
              : undefined
          }
        />
        <ProblemList
          title="Странные данные"
          objects={strangeObjects}
          emptyLabel="Подозрительных описаний не найдено."
          onEditObject={onEditObject}
          getReason={(object) => {
            const reasons: string[] = [];
            if (hasStrangeDescription(object.description)) {
              reasons.push('текст: «уточните у консультанта»');
            }
            return reasons.join(', ');
          }}
        />
      </div>
    </div>
  );
};
