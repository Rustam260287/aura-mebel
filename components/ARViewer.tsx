...
const ARViewerComponent = forwardRef<ARViewerHandle, ARViewerProps>(
  ({ src, iosSrc, alt, poster, objectId, open = false, onClose }, ref) => {
    ...
    const [env] = useState(() => typeof window !== 'undefined' ? getBrowserEnvironment() : null);
    ...
    return (
      ...
      >
        ...
        {/* Strict Guard: If external browser required, do not even attempting to render model-viewer */}
        {env?.requiresExternalBrowser ? (
          <ArBrowserGuard onClose={() => onClose?.()}>
            {/* No content needed, Guard will render fallback UI */}
            <></>
          </ArBrowserGuard>
        ) : (
          <ArBrowserGuard onClose={() => onClose?.()}>
