...
import { generateImprovedObjectDescription } from '../../../../services/ai';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const db = getAdminDb();
  const limitedIds = objectIds.slice(0, 30);

  const results: { id: string; status: 'ok' | 'skipped' | 'error'; message?: string }[] = [];

  for (const id of limitedIds) {
    try {
      const docRef = db.collection(COLLECTIONS.objects).doc(id);
      ...
      const data = snapshot.data() as ObjectAdmin;
      if (!data?.name) {
      ...
      const improved = await generateImprovedObjectDescription({
        name: data.name,
        objectType: (data as any).objectType || (data as any).category || '',
      ...
      console.error(`Bulk description error for object ${id}:`, error);
