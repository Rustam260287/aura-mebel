...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<{ posts: JournalSummary[] } | { error: string }>
) {
  const db = getAdminDb();
  if (!db) {
  ...
  try {
    ...
    console.error('Failed to load journal summaries', error);
    return res.status(500).json({ error: 'Failed to load journal list' });
  }
}
